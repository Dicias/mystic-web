...
// MySaC brand palette — red / yellow / black particles instead of monochrome
const COLOR_SCHEME = {
  light: {
    particles: [
      'rgba(224, 35, 28, 0.16)', // brand-red
      'rgba(255, 212, 0, 0.16)', // brand-yellow
      'rgba(13, 13, 13, 0.1)', // brand-black
    ],
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (const particle of particles) {
        ...
        const opacityFactor = Math.sin((particle.life / particle.maxLife) * Math.PI)
        ...
        const [r, g, b, a] = scheme.particles[particle.colorIndex]
          .replace(/[^\d.,]/g, '')
          .split(',')
          .map(Number)
        ctx.fillStyle = `rgba(${r}, ${g}, ${b}, ${a * opacityFactor})`
        ctx.beginPath()
